function validator(options) {
    ...
    function validate(inputElement, rule, errorElement) {
        ...
        if (errorMessage) {
            ...
            getParent(inputElement, options.getParentForm).classList.add(
                "invalid"
            );
        } else {
            errorElement.innerText = "";
            getParent(inputElement, options.getParentForm).classList.remove(
                "invalid"
            );
        }
        ...
        formElement.onsubmit = (e) => {
            ...
            if (isFormValid) {
                if (typeof options.onSubmit === "function") {
                    var inputValues =
                        formElement.querySelectorAll("input[name]");
                    var formValues = Array.from(inputValues).reduce(
                        (values, input) => {
                            switch (input.type) {
                                case "radio":
                                    values[input.name] =
                    ...
                    console.log(
                        `This is ${options.onSubmit} is not a function!`
                    );
                }
    ...
    return {
        ...
        test: function (value) {
            return value === getConfirm()
                ? undefined
                : message || "Trường nhập lại không chính xác";
        },
    };
};
